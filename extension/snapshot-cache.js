...
export class SnapshotCache {
  ...
  capture(tabId, raw, options = {}) {
    ...
    if (since) {
      ...
      else if (snapshot.truncated || previous.snapshot.truncated) resetReason = 'truncated_snapshot'
    }
    const snapshotId = this.id()
    this.entries.set(snapshotId, { tabId: String(tabId), documentKey, signature, snapshot, time })
    ...
    const newElements = new Map(snapshot.elements.map(el => [el.locator, el]))
    ...
    return { mode: 'diff', snapshotId, since, reset: false, title: snapshot.title, ready: snapshot.ready,
      ...
      truncated: snapshot.truncated, scanTruncated: snapshot.scanTruncated, added, changed, removed,
